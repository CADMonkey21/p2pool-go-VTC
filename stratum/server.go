...
func (s *StratumServer) handleAuthorize(c *Client, req *JSONRPCRequest) {
	...
	// [FIX] Normalize Bech32 addresses to lowercase to ensure they match
	// the format generated by the ShareChain payout calculator.
	if strings.HasPrefix(strings.ToLower(addr), "vtc") || strings.HasPrefix(strings.ToLower(addr), "tvtc") {
