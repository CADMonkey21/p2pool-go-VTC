...
func (s *StratumServer) handleSubmit(c *Client, req *JSONRPCRequest) {
	...
	if req.ID == nil || string(*req.ID) == "null" {
		respID = 0 // Legacy p2pool answers with numeric 0 on null-id submits
	} else {
		respID = json.RawMessage(*req.ID)
	}
	
	response := JSONRPCResponse{ID: respID, Result: shareAccepted, Error: nil}
