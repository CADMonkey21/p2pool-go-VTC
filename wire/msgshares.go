...
func (s *Share) FromBytes(r io.Reader) error {
	...
	err = s.Contents.FromBytes(contentsReader, s.Type)

	// If it fails, the calling function (ReadShares) will now log the hex and handle it.
	if err != nil {
		return err
	}
...
func (sc *ShareContents) FromBytes(r io.Reader, shareType uint64) error {
	...
	sc.MinHeader.PreviousBlock, err = ReadPossiblyNoneHash(r) // CORRECTED
	if err != nil {
	...
	sc.ShareInfo.ShareData.PreviousShareHash, err = ReadPossiblyNoneHash(r) // CORRECTED
	if err != nil {
	...
	// 2b. Read SegwitData - STRICT LOGIC
	if shareType >= 16 {
		...
		// Check if it's the explicit "None" value
		isNoneValue := false
	...
	sc.ShareInfo.FarShareHash, err = ReadPossiblyNoneHash(r) // CORRECTED
	if err != nil {
...
func (m *MsgShares) FromBytes(b []byte) error {
	...
	// The error from ReadShares is now the high-level error, if any.
	// Individual share errors are handled inside ReadShares.
	if err != nil {
